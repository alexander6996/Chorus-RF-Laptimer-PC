...
import com.chorusrflaptimer.wrapper.TextUtils;
...
public class AppPreferences {
    ...
    public static void save(String preferenceName) {
        ...
        switch (preferenceName) {
            case SKIP_FIRST_LAP:
            	SharedPreferences.putBoolean(SKIP_FIRST_LAP, app.shouldSkipFirstLap);
                break;
            case SPEAK_LAP_TIMES:
            	SharedPreferences.putBoolean(SPEAK_LAP_TIMES, app.shouldSpeakLapTimes);
                break;
            case SPEAK_MESSAGES:
            	SharedPreferences.putBoolean(SPEAK_MESSAGES, app.shouldSpeakMessages);
                break;
            case SPEAK_ENGLISH_ONLY:
            	SharedPreferences.putBoolean(SPEAK_ENGLISH_ONLY, app.shouldSpeakEnglishOnly);
                break;
            case LAPS_TO_GO:
                if (app.raceState == null) break;
                SharedPreferences.putInt(LAPS_TO_GO, app.raceState.lapsToGo);
                break;
            case MIN_LAP_TIME:
                if (app.raceState == null) break;
                SharedPreferences.putInt(MIN_LAP_TIME, app.raceState.minLapTime);
                break;
            case PREPARATION_TIME:
            	SharedPreferences.putInt(PREPARATION_TIME, app.timeToPrepareForRace);
                break;
            case ENABLE_DEVICE_SOUNDS:
            	SharedPreferences.putBoolean(ENABLE_DEVICE_SOUNDS, app.isDeviceSoundEnabled);
                break;
            case DEVICE_BANDS:
                ...
                SharedPreferences.putString(DEVICE_BANDS, bands);
                break;
            case DEVICE_CHANNELS:
                ...
                SharedPreferences.putString(DEVICE_CHANNELS, channels);
                break;
            case DEVICE_PILOTS:
                if (app.deviceStates == null) break;
                ...
                SharedPreferences.putString(DEVICE_PILOTS, pilots);
                break;
            case DEVICE_ENABLED:
                if (app.deviceStates == null) break;
                ...
                SharedPreferences.putString(DEVICE_ENABLED, statuses);
                break;
            case DEVICE_THRESHOLDS:
                ...
                SharedPreferences.putString(DEVICE_THRESHOLDS, thresholds);
                break;

            case LIPO_MONITOR_ENABLED:
            	SharedPreferences.putBoolean(LIPO_MONITOR_ENABLED, app.isLiPoMonitorEnabled);
                break;
            case LIPO_ADJUSTMENT_CONST:
            	SharedPreferences.putInt(LIPO_ADJUSTMENT_CONST, app.batteryAdjustmentConst);
                break;
            case BLUETOOTH_LAST_CONNECT:
            	if(app.bluetoothLastConnect != null) {
            		SharedPreferences.putString(BLUETOOTH_LAST_CONNECT, app.bluetoothLastConnect[0]+"##"+app.bluetoothLastConnect[1]);	
            	}
                break;
    ...
    public static void applyInAppPreferences() {
        ...
        String value = app.preferences.getString(BLUETOOTH_LAST_CONNECT, null);
        if(value != null) {
        	String[] valueTemp = value.split("##");
        	app.bluetoothLastConnect = valueTemp;
        }
