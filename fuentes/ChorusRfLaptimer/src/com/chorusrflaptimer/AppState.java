...
import com.chorusrflaptimer.wrapper.ToneGenerator;
...
public class AppState {
    ...
    public static final int BATTERY_CHECK_INTERVAL = 5000; // 10000 - 10 seconds
    ...
    public String[] bluetoothLastConnect;

    private ArrayList<Boolean> deviceTransmissionStates;
    private ArrayList<IDataListener> mListeners;
    private ToneGenerator mToneGenerator;
    public Handler mBatteryMonitorHandler;
    public Handler mBatteryNotifyHandler;
            ...
            public void handleMessage(Message msg) {
                ...
                if (app.isConnected && app.isLiPoMonitorEnabled) {
                    if (batteryPercentage <= 10) {
                        speakMessage(UtilMessage.getMsg("battery_status_critical"));
                    } else if (batteryPercentage <= 20){
                        speakMessage(UtilMessage.getMsg("battery_status_low"));
                    }
                }
...
//        Log.i("SEND BT COMMAND", cmd);
    ...
    public void speakMessage(int msg) {
        if (shouldSpeakMessages) {
            textSpeaker.speak(msg);
        }
    }
    ...
    public void changeRaceState(boolean isStarted) {
        ...
        if (raceState.isStarted!= isStarted) {
            ...
            if (!isStarted && isDevicesInitializationOver()) {
                speakMessage(UtilMessage.getMsg("race_announcement_finished"));
        ...
        if (isDevicesInitializationOver()) {
            ...
            if (shouldSpeakLapTimes) {
                ...
                if (isJustFinished(deviceId)) {
                    textSpeaker.speak(UtilMessage.getMsg("race_pilot_finished"), pilotName);
                } else if (getIsFinished(deviceId)) {
                    textSpeaker.speak(UtilMessage.getMsg("race_pilot_already_finished"), pilotName);
                } else {
                ...
                textSpeaker.speak(UtilMessage.getMsg("race_lap_report"), lapNumberToSpeak);
	...
	public void setLanguageApp(String LanguageApp) {
